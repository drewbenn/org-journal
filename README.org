...
- =C-c C-S-f= - go to the next journal display unit, if enabled.

- =C-c C-S-b= - go to the previous journal display unit, if enabled.
...
- =org-journal-display-type= - the journal display type either nil
  (not enabled, (default), 'weekly or 'monthly. When enabled, the
  display is narrowed to this view so you can, for example, view only
  a week at a time while storing all the entries for a year in a
  single file. If you are in this mode and want to see the full file,
  use `C-x n w` or `M-x widen`.

  If you set this to 'weekly, weeks that split across your file-type
  'monthly or 'yearly boundary will only be visible as separate weeks:
  this won't build a combined view from 2 different files.

  There is no display-type analogue for =org-journal-file-header=, so
  you can't insert something when you create a new journal entry in a
  new display type.
...
Yes, see =org-journal-file-type= and =org-journal-display-type=.
